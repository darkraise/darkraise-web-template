...
function ChartStyle({ id, config }: { id: string; config: ChartConfig }) {
  ...
  return (
    <style
      // CSS is derived from developer-controlled ChartConfig keys and values,
      // not user-supplied input, so dangerouslySetInnerHTML is safe here.
        ...
        {payload.map((item, index) => {
          ...
          return (
            <div
              ...
              className={cn(
                "[&>svg]:text-muted-foreground flex w-full flex-wrap items-stretch gap-2 [&>svg]:size-[var(--icon-size-2xs)]",
      ...
      {payload.map((item) => {
        ...
        return (
          <div
            key={item.value}
            className={cn(
              "[&>svg]:text-muted-foreground flex items-center gap-1.5 [&>svg]:size-[var(--icon-size-xs)]",
            )}
          >
